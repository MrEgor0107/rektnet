...
def vis_tensor_and_save(image, h, w, tensor_output, image_name, output_uri):
    colors = [(0, 255, 0), (255, 0, 0), (255, 255, 0), (0, 255, 255), (255, 0, 255), (127, 255, 127), (255, 127, 127), (255, 127, 127), (255, 127, 127), (255, 127, 127), (255, 127, 127)]
    i = 0
...
def calculate_mean_distance(epoch_kpt_dis):
    ...
    for i, dist in enumerate(epoch_kpt_dis):
        top.append(dist[0])
        first_L_top.append(dist[1])
        first_R_top.append(dist[2])
        first_L_bot.append(dist[3])
        first_R_bot.append(dist[4])
        second_L_top.append(dist[5])
        second_R_top.append(dist[6])
        second_L_bot.append(dist[7])
        second_R_bot.append(dist[8])
        bot_L.append(dist[9])
        bot_R.append(dist[10])

  
    top_std = np.std(top)
    top = np.mean(top)
    
    first_L_top_std = np.std(first_L_top)
    first_L_top = np.mean(first_L_top)
    
    first_R_top_std = np.std(first_R_top)
    first_R_top = np.mean(first_R_top)
    
    first_L_bot_std = np.std(first_L_bot)
    first_L_bot = np.mean(first_L_bot)
    
    first_R_bot_std = np.std(first_R_bot)
    first_R_bot = np.mean(first_R_bot)

    second_L_top_std = np.std(second_L_top)
    second_L_top = np.mean(second_L_top)
    
    second_R_top_std = np.std(second_R_top)
    second_R_top = np.mean(second_R_top)
    
    second_L_bot_std = np.std(second_L_bot)
    second_L_bot = np.mean(second_L_bot)
    
    second_R_bot_std = np.std(second_R_bot)
    second_R_bot = np.mean(second_R_bot)
    ...
    total = top + first_L_top + first_R_top + first_L_bot + first_R_bot + second_L_top + second_R_top + second_L_bot + second_R_bot + bot_L + bot_R

    return [top,first_L_top,first_R_top,first_L_bot,first_R_bot,second_L_top,second_R_top,second_L_bot,second_R_bot,bot_L,bot_R],total,[top_std,first_L_top_std,first_R_top_std,first_L_bot_std,first_R_bot_std,second_L_top_std,second_R_top_std,second_L_bot_std,second_R_bot_std,bot_L_std,bot_R_std]
