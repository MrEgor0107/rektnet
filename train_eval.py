...
from keypoint_net_big import KeypointNetBig
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            if save_checkpoints:
                ...
                onnx_model = KeypointNetBig(num_kpt, input_size, onnx_mode=True)
...
def main():
    ...
    KPT_KEYS = ["top", "first_L_top", "first_R_top", "first_L_bot", "first_R_bot", "second_L_top", "second_R_top", "second_L_bot", "second_R_bot", 'bot_L', 'bot_R']
    ...
    model = KeypointNetBig(len(KPT_KEYS), INPUT_SIZE, onnx_mode=False)
    model = model.to(device)
    optimizer = optim.Adam(model.parameters(), lr=args.lr)
    scheduler = optim.lr_scheduler.ExponentialLR(optimizer, gamma=args.lr_gamma)
    loss_func = CrossRatioLossBig(args.loss_type, args.geo_loss, args.geo_loss_gamma_horz, args.geo_loss_gamma_vert)
