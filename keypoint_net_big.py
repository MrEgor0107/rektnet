...
class KeypointNetBig(nn.Module):
    def __init__(self, num_kpt=11, image_size=(80, 80), onnx_mode=False, init_weight=True): # num_kpt было = 7 
        super(KeypointNetBig, self).__init__()
        net_size = 16
        ...
        self.relu = nn.ReLU()
        self.res1 = ResNetBig(net_size, net_size)
        self.res2 = ResNetBig(net_size, net_size * 2)
        self.res3 = ResNetBig(net_size * 2, net_size * 4)
        self.res4 = ResNetBig(net_size * 4, net_size * 8)
...
if  __name__=='__main__':
    ...
    loss = CrossRatioLossBig()
    target = autograd.Variable(torch.randn(3, 11, 2))
    l = loss(test, target)
